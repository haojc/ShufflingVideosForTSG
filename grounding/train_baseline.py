...
def perpare_data(batch_data):
    ...
    video_feat, nfeats, video_mask, gt = batch_data
...
def constract_model(params, logger):
    ...
    model = Baseline(video_seq_set, sent_seq_set, grounding_set, matching_set, logger, params['dropout'])
...
def train(model, data_loader, params, logger, step, optimizer, dataset):
    model.train()
    ...
    for idx, batch_data in enumerate(data_loader):
        ...
        sent_list, sent_feat, sent_len, sent_mask, \
        video_duration, vid_list, \
        video_feat, nfeats, video_mask, gt = perpare_data(batch_data)

        span_prob = model(
            video_feat, sent_feat,
            video_mask, sent_mask,
        )
...
def valid(model, data_loader, params, logger, step, saver, dataset):
    ...
    for idx, batch_data in enumerate(data_loader):
        ...
        with torch.no_grad():
            ...
            # LOSS
            # Grounding Loss
            loss = span_ground_loss(span_prob['start'], span_prob['end'], gt['framestps'])
...
def select_dataset_and_cfn(dataset_name):
    if dataset_name in ['charades', 'charades_cd']:
        from dataset.charades import collate_fn, CharadesDataSentence
        data_class = CharadesDataSentence
        cfn = collate_fn
    elif dataset_name in ['anet', 'anet_cd']:
        from dataset.anet import collate_fn, ANetDataSentence
        data_class = ANetDataSentence
        cfn = collate_fn
    else:
...
def main(params):
    ...
    for step in range(params['epoch']):
        # train
        loss = train(model, train_loader, params, logger, step, optimizer, train_set)
        lr_scheduler.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--if_aug', action='store_true', default=False,
                        help='data augment')
    parser.add_argument('--aug_percentage', type=float, default=0,
                        help='aug_percentage')
    ...
    parser.add_argument('--lr_step', type=int, nargs='+', default=[20, 30],
